ProfileManager = {
    ...
    init: function ()
    {
        $('#user_info_is_vocalist').on('ifChecked', function(e) {
            $('.vocal-profile').slideDown();
            $('#vocalist-fee').removeClass('hide');
            ProfileManager.userCheckAccountType();
        });
        $('#user_info_is_vocalist').on('ifUnchecked', function(e) {
            $('.vocal-profile').slideUp();
            $('#vocalist-fee').addClass('hide');
            ProfileManager.userCheckAccountType();
        });
        $('#user_info_is_producer').on('ifChecked', function(e) {
            $('#producer-fee').removeClass('hide');
            ProfileManager.userCheckAccountType();
        });
        $('#user_info_is_producer').on('ifUnchecked', function(e) {
            $('#producer-fee').addClass('hide');
            ProfileManager.userCheckAccountType();
        });
        if (!$('#user_info_is_vocalist').is(':checked')) {
            $('.vocal-profile').hide(); 
        }
    ...
    userCheckAccountType: function () {
        if (!$('#user_info_is_vocalist').is(':checked') && !$('#user_info_is_producer').is(':checked')) {
            $('#fees-wrap').hide();
        }
        else {
            $('#fees-wrap').show();
