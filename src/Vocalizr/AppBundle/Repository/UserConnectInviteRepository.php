...
use Vocalizr\AppBundle\Exception\UpgradeException;
...
class UserConnectInviteRepository extends EntityRepository
{
    ...
    public function isConnectAllowed($fromUser, $toUser)
    {
        ...
        $blockUser = $em->getRepository('VocalizrAppBundle:UserBlock')->findOneBy([
            'user_info'  => $toUser,
            'block_user' => $fromUser,
        ]);
        if ($blockUser) {
        ...
        $subscriptionPlan = $em->getRepository('VocalizrAppBundle:SubscriptionPlan')->getActiveSubscription($fromUser->getId());

        // Check how many connections made this month
        $connectCount = $em->getRepository('VocalizrAppBundle:Counter')->getCount($fromUser, Counter::TYPE_CONNECT);

        if ($connectCount >= $subscriptionPlan['connect_month_limit']) {
            if ($subscriptionPlan['static_key'] == \Vocalizr\AppBundle\Entity\SubscriptionPlan::PLAN_FREE) {
