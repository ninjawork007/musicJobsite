...
use Vocalizr\AppBundle\Entity\ProjectInvite;
...
use Vocalizr\AppBundle\Entity\UserConnect;
use Vocalizr\AppBundle\Entity\UserConnectInvite;
...
use Vocalizr\AppBundle\Entity\UserVoiceTagVote;
...
//            $em->persist($entity->getEmployeeUserInfo());
    ...
    private function ProjectAssetPostPersist($em, $entity)
    {
        ...
        $activity = $em->getRepository('VocalizrAppBundle:ProjectActivity')
    ...
    private function ProjectBidPostPersist($em, $entity)
    {
        ...
        $qb = $em->getRepository('VocalizrAppBundle:ProjectBid')
    ...
    private function ProjectInvitePostPersist($em, $entity)
    {
        ...
        // Add notification
        $n = new \Vocalizr\AppBundle\Entity\Notification();
        ...
        if ($entity->getProject()->getHireUser()) {
            $n->setNotifyType(\Vocalizr\AppBundle\Entity\Notification::NOTIFY_TYPE_PROJECT_HIRE);
        } else {
            $n->setNotifyType(\Vocalizr\AppBundle\Entity\Notification::NOTIFY_TYPE_PROJECT_INVITE);
        }
        $em->persist($n);
    ...
    private function UserConnectInvitePostPersist($em, $entity)
    {
        ...
        $n->setNotifyType(\Vocalizr\AppBundle\Entity\Notification::NOTIFY_TYPE_CONNNECT_INVITE);
        $em->persist($n);
    ...
    private function UserConnectInvitePostRemove($em, $entity)
    {
        ...
        $qb = $em->getRepository('VocalizrAppBundle:Notification')->createQueryBuilder('n');
        $qb->delete('VocalizrAppBundle:Notification', 'n');
        ...
        if ($entity->getConnectedAt()) {
            $qb = $em->getRepository('VocalizrAppBundle:UserConnect')->createQueryBuilder('uc');
            ...
            $uc1 = $em->getRepository('VocalizrAppBundle:UserConnect')->findOneBy([
                'to'   => $entity->getTo(),
                'from' => $entity->getFrom(),
            ]);
            if ($uc1) {
            ...
            $uc2 = $em->getRepository('VocalizrAppBundle:UserConnect')->findOneBy([
                'from' => $entity->getTo(),
                'to'   => $entity->getFrom(),
            ]);
            if ($uc2) {
    ...
    private function ProjectBidPostRemove($em, $entity)
    {
        ...
        $qb = $em->getRepository('VocalizrAppBundle:ProjectActivity')
