...
class MessageController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        if (count($threads) > 0) {
            ...
            $message = new \Vocalizr\AppBundle\Entity\Message();
            ...
            $response = $this->render(
                'VocalizrAppBundle:Message:index.html.twig',
                ['threads'            => $threads,
            ...
            $response = $this->render('VocalizrAppBundle:Message:empty.html.twig');
        }
        return $response;
    }
    ...
    public function viewMessagesAction(Request $request, $uuid)
    {
        ...
        $thread = $em->getRepository('VocalizrAppBundle:MessageThread')
                ->findOneBy(['uuid' => $uuid]);
        ...
        // get the bid for the active thread
        $threadBid = $em->getRepository('VocalizrAppBundle:ProjectBid')
        ...
        return new Response(json_encode([
            ...
            'html'    => $this->renderView(
                'VocalizrAppBundle:Message:threadMessages.html.twig',
                $templateData
            ),
        ]));
    }
    ...
    public function replyAction(Request $request, $uuid)
    {
        ...
        $messageThread = $em->getRepository('VocalizrAppBundle:MessageThread')
                    ->findOneBy(['uuid' => $uuid]);
        ...
        if ($request->isMethod('POST')) {
            ...
            $messageFileRepo = $em->getRepository('VocalizrAppBundle:MessageFile');
    ...
    public function composeAction(Request $request, $projectUuid, $userId)
    {
        ...
        $toUser = $em->getRepository('VocalizrAppBundle:UserInfo')
                     ->findOneBy(['id' => $userId]);
        if (!$toUser) {
            return $this->forward('VocalizrAppBundle:Default:error', [
                'error' => 'Invalid user',
        ...
        $bid = $em->getRepository('VocalizrAppBundle:ProjectBid')
        ...
        if ($request->isMethod('POST')) {
            ...
            if (!$messageThread) {
                ...
                $messageThread = new \Vocalizr\AppBundle\Entity\MessageThread();
            ...
            if (!$toUserPref) {
                $toUserPref = new \Vocalizr\AppBundle\Entity\UserPref();
            }
            ...
            if ($toUserPref->getEmailMessages()) {
                ...
                $body = $this->container->get('templating')->render('VocalizrAppBundle:Mail:newMessage.html.twig', [
                    'toUserInfo'    => $toUser,
                    'fromUserInfo'  => $user,
                    'project'       => $project,
    ...
    public function composePrivateAction(Request $request, $userId)
    {
        ...
        $userConnect = $em->getRepository('VocalizrAppBundle:UserConnect')
                ->findOneBy([
                    'to'   => $toUser,
                    'from' => $user,
                ]);
        ...
        $userBlocked = $em->getRepository('VocalizrAppBundle:UserBlock')
                ->findOneBy([
                    'user_info'  => $toUser,
                    'block_user' => $user,
                ]);
        ...
        $messageCount     = $em->getRepository('VocalizrAppBundle:Counter')->getCount($user, Counter::TYPE_MESSAGE);
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($toUserPref->getEmailMessages()) {
                ...
                $body = $this->container->get('templating')->render('VocalizrAppBundle:Mail:newMessagePrivate.html.twig', [
                    'toUserInfo'    => $toUser,
                    'fromUserInfo'  => $user,
                    'messageThread' => $messageThread,
                    'message'       => $message,
            ...
            $em->getRepository('VocalizrAppBundle:Counter')->addCount($user, Counter::TYPE_MESSAGE);
    ...
    public function deleteFileAction(Request $request)
    {
        ...
        // Get slug
        $messageFile = $em->getRepository('VocalizrAppBundle:MessageFile')->
    ...
    public function warningAction($projectUuid, $userId)
    {
        ...
        if ($session->get('messageWarningSeen')) {
            return $this->forward('VocalizrAppBundle:Message:compose', ['projectUuid' => $projectUuid, 'userId' => $userId]);
        }
        return [];
    }
    ...
    public function warningPrivateAction($userId)
    {
        ...
        if ($subscriptionPlan['message_month_limit']) {
            ...
            if ($messageCount >= $subscriptionPlan['message_month_limit']) {
                return $this->render('VocalizrAppBundle:Message:upgrade.html.twig', [
        ...
        if ($session->get('messageWarningSeen')) {
            return $this->forward('VocalizrAppBundle:Message:composePrivate', ['userId' => $userId]);
        }
