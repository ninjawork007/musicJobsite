...
use Vocalizr\AppBundle\Form\Type\UserSearchType;
use Vocalizr\AppBundle\Form\Type\VocalistSearchType;
...
class VocalistsController extends AbstractUserSearchController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $form = $this->createForm(new UserSearchType($this->getProjectConfigData()['budget'], $lockCertified));
        ...
        if ($request->get('search')) {
            if ($form->isValid()) {
                ...
                if ($data['country']) {
                    $country = $em->getRepository('VocalizrAppBundle:Country')
                    ...
                    if ($endEmpty) {
                            ...
                            $q->expr()->orX(
                                $q->expr()->in(
                                    ...
                                    $em->createQueryBuilder()
                                        ->select('uui1.id')
                                        ->from('Vocalizr\AppBundle\Entity\UserInfoLanguage', 'ul1')
                                ...
                                $q->expr()->notIn(
                                    ...
                                    $em->createQueryBuilder()
                                        ->select('uui2.id')
                                        ->from('Vocalizr\AppBundle\Entity\UserInfoLanguage', 'ul2')
                        ...
                        $q->andWhere(
                            $expr->in(
                                ...
                                $em->createQueryBuilder()
                                    ->select('uui.id')
                                    ->from('Vocalizr\AppBundle\Entity\UserInfoLanguage', 'ul')
        ...
        // Top tags
        $topSoundsLike = $em->getRepository('VocalizrAppBundle:UserVoiceTag')->getTop10();

        $freePlan = $em->getRepository('VocalizrAppBundle:SubscriptionPlan')->findOneBy([
            'static_key' => 'FREE',
        ]);

        $hasProjects = $em->getRepository('VocalizrAppBundle:Project')
                ->getUserOpenProjectCount($user);

        return $this->render('@VocalizrApp/Vocalists/index.html.twig', [
            'favs'             => $favs,
     ...
     * @Template()
     */
    public function maleVocalistsAction(Request $request)
    {
        $em           = $this->getDoctrine()->getManager();
        $user         = $this->getUser();
        $userInfoRepo = $em->getRepository('VocalizrAppBundle:UserInfo');
     ...
     * @Template()
     */
    public function femaleVocalistsAction(Request $request)
    {
        $em           = $this->getDoctrine()->getManager();
        $user         = $this->getUser();
        $userInfoRepo = $em->getRepository('VocalizrAppBundle:UserInfo');
     ...
     */
    public function sessionSingersHireAction(Request $request)
    {
        $em           = $this->getDoctrine()->getManager();
        $user         = $this->getUser();
        $userInfoRepo = $em->getRepository('VocalizrAppBundle:UserInfo');
     ...
     * @param \Symfony\Component\HttpFoundation\Request $request
     */
    public function femaleSingersHireAction(Request $request)
    {
        $em           = $this->getDoctrine()->getManager();
        $user         = $this->getUser();
        $userInfoRepo = $em->getRepository('VocalizrAppBundle:UserInfo');
     ...
     * @param \Symfony\Component\HttpFoundation\Request $request
     */
    public function maleSingersHireAction(Request $request)
    {
        $em           = $this->getDoctrine()->getManager();
        $user         = $this->getUser();
        $userInfoRepo = $em->getRepository('VocalizrAppBundle:UserInfo');
