...
use Vocalizr\AppBundle\Entity\MarketplaceItem;

use Vocalizr\AppBundle\Entity\MarketplaceItemAudio;

use Vocalizr\AppBundle\Form\Type\MarketplaceItemType;
...
class MarketplaceController extends Controller
{
    ...
    public function createAction(Request $request)
    {
        ...
        $marketplaceItemAudioRepo = $em->getRepository('VocalizrAppBundle:MarketplaceItemAudio');
    ...
    public function editAction(Request $request, $uuid)
    {
        ...
        $itemAudioRepo = $em->getRepository('VocalizrAppBundle:MarketplaceItemAudio');
    ...
    public function audioAction(Request $request)
    {
        ...
        $marketplaceItem = $em->getRepository('VocalizrAppBundle:MarketplaceItem')
                    ->findOneByUuid($uuid);
        ...
        $itemAudio = $em->getRepository('VocalizrAppBundle:MarketplaceItemAudio')->findOneBy(['slug' => $request->get('slug')]);
    ...
    public function deleteAssetAction(Request $request, $uuid, $slug)
    {
        ...
        $asset = $em->getRepository('VocalizrAppBundle:MarketplaceItemAsset')->getBySlug($slug);
        ...
        if (count($files) > 0 || count($dbfiles) > 0) {
            ...
            if (count($files) > 0) {
                foreach ($files as $i => $file) {
                    ...
                    if (!$marketplaceItemAsset = $em->getRepository('VocalizrAppBundle:MarketplaceItemAsset')
            ...
            if ($dbFileNames = $request->get('dropbox_file_name')) {
                ...
                foreach ($dbFileNames as $k => $name) {
                    ...
                    $marketplaceItemAsset = $em->getRepository('VocalizrAppBundle:MarketplaceItemAsset')
    ...
    public function audioMarketplaceAssetAction(Request $request)
    {
        ...
        if (!$asset = $em->getRepository('VocalizrAppBundle:MarketplaceItemAsset')->findOneBy(['slug' => $request->get('slug')])) {
            throw $this->createNotFoundException();
        }
    ...
    public function userMarketplaceAction(Request $request)
    {
        ...
        // load the marketplace item
        $items = $em->getRepository('VocalizrAppBundle:MarketplaceItem')->findAll([], ['created_at' => 'DESC', 'updated_at' => 'DESC']);
