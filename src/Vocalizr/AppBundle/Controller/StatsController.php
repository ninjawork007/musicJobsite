...
class StatsController extends Controller
{
     ...
     * @Template()
     */
    public function playsAction(Request $request)
    {
        $user   = $this->getUser();
        ...
        $audioRepo = $em->getRepository('VocalizrAppBundle:UserAudio');
        ...
        $audioPlayUsers = $dm->createQueryBuilder('VocalizrAppBundle:AudioPlayUser')
            ->field('user_id')->equals($user->getId())
    ...
    public function likesAction(Request $request)
    {
        ...
        $audioLikeUsers = $dm->createQueryBuilder('VocalizrAppBundle:AudioLike')
            ->field('user_id')->equals($user->getId())
            ->field('date')->gte(date('Y-m-d', strtotime('-90 days')))
    ...
    public function viewsAction(Request $request)
    {
        ...
        if ($filter == '12months') {
            ...
            $profileViews = $dm->createQueryBuilder('VocalizrAppBundle:ProfileView')
                ->field('user_id')->equals($user->getId())
        ...
        // Get who has been liking the audio
        $profileViewUsers = $dm->createQueryBuilder('VocalizrAppBundle:ProfileViewUser')
            ->field('user_id')->equals($user->getId())
