...
use Vocalizr\AppBundle\Entity\EntryVote;
...
class ContestController extends Controller
{
    ...
    public function newAction(Request $request, $uuid)
    {
        ...
        $form         = $this->createForm(new NewProjectContestType($english, $projectYml['contest_budget']), $project);
    ...
    public function viewAction(Request $request)
    {
        ...
        if ($this->getUser()) {
            ...
            $projectAudioDownload = $em->getRepository('VocalizrAppBundle:ProjectAudioDownload')->
        ...
        if ($entry) {
            ...
            $entryBid = $em->getRepository('VocalizrAppBundle:ProjectBid')->getContestBidOne($entry, $project);
        }
        ...
        if ($project->getSfs()) {
            ...
            // get vote entries
            $qb = $em->getRepository('VocalizrAppBundle:EntryVote')
    ...
    public function bidsAction($uuid, $page, $filter, $dir)
    {
        ...
        $bids      = $em->getRepository('VocalizrAppBundle:ProjectBid')->getContestBids($project->getId(), $filters);
        $totalBids = $em->getRepository('VocalizrAppBundle:ProjectBid')->getContestTotalBids($project->getId(), $filters);
    ...
    public function editAction(Request $request)
    {
        ...
        $form         = $this->createForm(new \Vocalizr\AppBundle\Form\Type\EditProjectContestType($english), $project);
        $locationForm = $this->createForm(new LocationType());
                ...
                if ($form->isValid() && $locationForm->isValid()) {
                    ...
                    if ($request->get('audio_file')) {
                        ...
                        if ($defaultProjectAudio) {
                            ...
                            $q = $em->getRepository('VocalizrAppBundle:ProjectAudioDownload')
    ...
    public function projectStatusWidgetAction(Request $request, $uuid)
    {
        ...
        // Get bids & bid stats (total bids, avg bid amount)
        $bids     = $em->getRepository('VocalizrAppBundle:ProjectBid')->getProjectBids($project->getId());
        $bidStats = $em->getRepository('VocalizrAppBundle:ProjectBid')->getBidStats($project->getId());
        ...
        if ($user && ($project->isOwner($user) || $user->getIsAdmin())) {
            $defaultProjectAudio = $em->getRepository('VocalizrAppBundle:ProjectAudio')->findOneBy([
                'project' => $project->getId(),
                'flag'    => ProjectAudio::FLAG_FEATURED,
            ]);
            $audioDownloads = $em->getRepository('VocalizrAppBundle:ProjectAudioDownload')->findBy(['project_audio' => $defaultProjectAudio]);
        }
    ...
    public function audioDownloadAction(Request $request, $uuid)
    {
        ...
        // Get project featured audio
        $audio = $em->getRepository('VocalizrAppBundle:ProjectAudio')->findOneBy([
            'project' => $project->getId(),
            'flag'    => ProjectAudio::FLAG_FEATURED,
        ]);
        if (!$audio) {
        ...
        if ($audio->getUserInfo()->getId() != $user->getId()) {
            // Record download
            $pad = $em->getRepository('VocalizrAppBundle:ProjectAudioDownload')
                    ->findOneBy(['user_info' => $user, 'project_audio' => $audio]);
            if (!$pad) {
                $audio->setDownloadCount($audio->getDownloadCount() + 1);
                $em->persist($audio);

                $pad = new \Vocalizr\AppBundle\Entity\ProjectAudioDownload();
    ...
    private function notifyFavorites($project)
    {
        ...
        $userInfoFavs = $em->getRepository('VocalizrAppBundle:UserInfo')
        ...
        if ($userInfoFavs) {
            ...
            foreach ($favorites as $favUserInfo) {
                ...
                $q = $em->getRepository('VocalizrAppBundle:ProjectInvite')
                ...
                $pi = new \Vocalizr\AppBundle\Entity\ProjectInvite();
