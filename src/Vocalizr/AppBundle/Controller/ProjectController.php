...
use Vocalizr\AppBundle\Form\Type\EditProjectType;
use Vocalizr\AppBundle\Form\Type\LocationType;
use Vocalizr\AppBundle\Form\Type\NewProjectGigType;
use Vocalizr\AppBundle\Form\Type\ProjectBidType;
use Vocalizr\AppBundle\Form\Type\PublishType;
use Vocalizr\AppBundle\Model\HintModel;
use Vocalizr\AppBundle\Repository\ProjectBidRepository;
...
class ProjectController extends Controller
{
    ...
    public function newAction(Request $request, $uuid)
    {
        ...
        // new project form
        $english = $em->getRepository('VocalizrAppBundle:Language')
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid() && $locationForm->isValid()) {
                ...
                if (!$project->getId() && !$request->get('audio_file')) {
                    ...
                    return $this->render(
                        'VocalizrAppBundle:Project:new.html.twig',
                        [
    ...
    public function newHireAction(Request $request, $username)
    {
        ...
        // Get user
        $hireUser = $em->getRepository('VocalizrAppBundle:UserInfo')->findOneBy([
            'username'  => $username,
            'is_active' => true,
        ]);
        if (!$hireUser) {
        ...
        $userBlock = $em->getRepository('VocalizrAppBundle:UserBlock')
                ->findOneBy([
                    'block_user' => $user,
                    'user_info'  => $hireUser,
                ]);
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                ...
                // Add project invite
                $projectInvite = new \Vocalizr\AppBundle\Entity\ProjectInvite();
    ...
    public function newCancelAction(Request $request, $uuid)
    {
        ...
        $audio = $em->getRepository('VocalizrAppBundle:ProjectAudio')->findOneBy(['project' => $project]);
        if ($audio) {
            $em->remove($audio);
        }
        $em->remove($project);
    ...
    public function editAction(Request $request)
    {
        ...
        $favoriteCount     = $em->getRepository('VocalizrAppBundle:UserInfo')
        ...
        $bids                     = $em->getRepository('VocalizrAppBundle:ProjectBid')->getProjectBids($project->getId(), $filters);
        $bidStats                 = $em->getRepository('VocalizrAppBundle:ProjectBid')->getBidStats($project->getId());
        $bidStats['avgBidAmount'] = $bidStats['avgBidAmount'] / 100;

        $projectAwarded = $em->getRepository('VocalizrAppBundle:ProjectBid')->isProjectAwarded($project->getId());

        $projectContracts = $em->getRepository('VocalizrAppBundle:ProjectContract')->findBy([
            'project' => $project->getId(),
        ], [
            'created_at' => 'DESC',
        ]);
        ...
        $userBid = null;
        if ($user) {
            $userBid = $em->getRepository('VocalizrAppBundle:ProjectBid')->findOneBy([
    ...
    public function projectStatusWidgetAction(Request $request, $uuid)
    {
        ...
        $bidRepo = $em->getRepository('VocalizrAppBundle:ProjectBid');
        ...
        $defaultAudio = $em->getRepository('VocalizrAppBundle:UserAudio')->findOneBy([
    ...
    public function downloadContractAction(Request $request)
    {
        ...
        $projectContract = $em->getRepository('VocalizrAppBundle:ProjectContract')
                ->findOneBy([
    ...
    public function viewAction(Request $request)
    {
        ...
        if ($user && $user->getId() == $project->getUserInfo()->getId()) {
            $filters['owner'] = true;
            $shortlistBids    = $em->getRepository('VocalizrAppBundle:ProjectBid')->getContestShortlistBids($project);
            $hiddenBids       = $em->getRepository('VocalizrAppBundle:ProjectBid')->getContestHiddenBids($project);
        }
        $bids = $em->getRepository('VocalizrAppBundle:ProjectBid')->getProjectBids($project->getId(), $filters);
        ...
        $subscriptionPlan['free'] = $em->getRepository('VocalizrAppBundle:SubscriptionPlan')->findOneBy(['static_key' => 'FREE']);
        $subscriptionPlan['pro']  = $em->getRepository('VocalizrAppBundle:SubscriptionPlan')->findOneBy(['static_key' => 'PRO']);
    ...
    public function bidAction(Request $request)
    {
        ...
        $projectInvite = $em->getRepository('VocalizrAppBundle:ProjectInvite')
                ->findOneBy([
                    'project'   => $project->getId(),
                    'user_info' => $user->getId(),
                ]);
    ...
    public function awardAction(Request $request)
    {
        ...
        if ($projectAwarded = $em->getRepository('VocalizrAppBundle:ProjectBid')->isProjectAwarded($project->getId())) {
            throw $this->createNotFoundException('Gig has already been awarded');
        }
         ...
         */
        $projectBid = $em->getRepository('VocalizrAppBundle:ProjectBid')->getProjectBidByUuid($bidUuid);
        if (!$projectBid) {
        ...
        if ($request->getMethod() == 'POST' && $request->get('award')) {
            ...
            if ($form->isValid()) {
                ...
                $uwt = new \Vocalizr\AppBundle\Entity\UserWalletTransaction();
                ...
                $q = $em->getRepository('VocalizrAppBundle:MessageThread')->createQueryBuilder('mt');
                $q->update()
    ...
    public function awardConfirmAction(Request $request)
    {
        ...
        $projectBid = $em->getRepository('VocalizrAppBundle:ProjectBid')->getProjectBidByProjectId($bidUuid, $project->getId());
        if (!$projectBid) {
    ...
    public function jsonAction(Request $request)
    {
        ...
        if ($request->get('action') == 'loadLyrics') {
            $id            = $request->get('id');
            $projectLyrics = $em->getRepository('VocalizrAppBundle:ProjectLyrics')->find($id);
            $response->setData([
    ...
    public function deleteBidAction(Request $request)
    {
        ...
        $projectBid = $em->getRepository('VocalizrAppBundle:ProjectBid')
    ...
    public function inviteUserAction(Request $request)
    {
        ...
        if ($userPref = $userInfo->getUserPref()) {
            ...
            $sendEmail = \Vocalizr\AppBundle\Entity\UserPref::DEFAULT_EMAIL_PROJECT_INVITES;
        }
    ...
    public function inviteToGigButtonAction($userInfo, $projects = null, $hasProjects = true)
    {
        ...
        if (!$projects) {
            $projects = $em->getRepository('VocalizrAppBundle:Project')
    ...
    public function audioProjectAssetAction(Request $request)
    {
        ...
        if (!$projectAsset = $em->getRepository('VocalizrAppBundle:ProjectAsset')->findOneBy(['slug' => $request->get('slug')])) {
            throw $this->createNotFoundException();
        }
    ...
    public function audioProjectBidAction(Request $request)
    {
        ...
        if (!$projectBid = $em->getRepository('VocalizrAppBundle:ProjectBid')->findOneBy(['path' => $request->get('filename')])) {
            throw $this->createNotFoundException();
        }
    ...
    public function audioAction(Request $request)
    {
        ...
        $projectAudio = $em->getRepository('VocalizrAppBundle:ProjectAudio')->findOneBy(['slug' => $request->get('slug')]);
    ...
    private function markProjectRead($project)
    {
        ...
        if ($project->getUserInfo() == $this->getUser()) {
            ...
            $q       = $em->getRepository('VocalizrAppBundle:ProjectBid')->createQueryBuilder('pb');
            $q->update()
                    ->set('pb.read_at', ':now')
        ...
        $qb = $em->getRepository('VocalizrAppBundle:Notification')
        ...
        $em->getRepository('VocalizrAppBundle:Notification')->updateUnreadCount($this->getUser());
    ...
    public function deleteGigAction(Request $request, $uuid)
    {
        ...
        $projectActivities = $em->getRepository('VocalizrAppBundle:VocalizrActivity')->findBy([
            'project' => $project,
        ]);
