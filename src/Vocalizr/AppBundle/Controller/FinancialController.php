...
use Vocalizr\AppBundle\Form\Type\UserWithdrawType;
...
class FinancialController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $wtQuery = $em->getRepository('VocalizrAppBundle:UserWalletTransaction')
        ...
        // Get payment escrows
        $escrows = $em->getRepository('VocalizrAppBundle:ProjectEscrow')
                ->getEscrowsUserInvolved($user->getId());

        /** @var Session $session */
        $session = $request->getSession();

        if ($this->get('vocalizr_app.user_restriction')->canWithdrawInstantly()) {
    ...
    public function withdrawAction(Request $request)
    {
        ...
        if (!$this->get('vocalizr_app.user_restriction')->canWithdrawInstantly() && $session->getFlashBag()->get(MembershipSourceHelper::SUB_SOURCE_INSTANT_WITHDRAWALS_MODAL)) {
        ...
        if ($request->get('withdrawId')) {
            if (
                $withdrawToCancel = $em->getRepository('VocalizrAppBundle:UserWithdraw')
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($withdrawForm->isValid()) {
                ...
                } elseif (!$this->get('vocalizr_app.user_restriction')->canWithdrawOnEmail($newWithdraw->getPaypalEmail())) {
        ...
        $withdrawRequests = $em->getRepository('VocalizrAppBundle:UserWithdraw')
        ...
        return $this->render('@VocalizrApp/Financial/withdraw.html.twig', [
