...
use Vocalizr\AppBundle\Entity\UserVocalCharacteristicVote;
use Vocalizr\AppBundle\Entity\UserVocalStyleVote;
...
use Vocalizr\AppBundle\Exception\RestoreSubscriptionException;
use Vocalizr\AppBundle\Exception\UnsubscribeException;
use Vocalizr\AppBundle\Exception\UserConnectionNotAllowedException;
use Vocalizr\AppBundle\Form\Type\ProProfileType;
use Vocalizr\AppBundle\Form\Type\UserEmailType;
use Vocalizr\AppBundle\Form\Type\UserInfoType;
use Vocalizr\AppBundle\Form\Type\UserPasswordType;
use Vocalizr\AppBundle\Model\UserConnectModel;
use Vocalizr\AppBundle\Model\UserSpotifyPlaylistModel;
use Vocalizr\AppBundle\Model\UserVideoModel;
use Vocalizr\AppBundle\Object\MembershipSourceObject;
use Vocalizr\AppBundle\Repository\UserConnectInviteRepository;
...
class UserController extends Controller
{
    ...
    public function viewAction(Request $request)
    {
        ...
        if ($user) {
            ...
            if ($user->getId() != $userInfo->getId()) {
                $userConnect = $em->getRepository('VocalizrAppBundle:UserConnectInvite')
                        ->findOneBy([
                            'to'   => $user,
                            'from' => $userInfo,
                        ]);
                ...
                $isUserFavorite = $em->getRepository('VocalizrAppBundle:UserInfo')
                        ->isUserFavorite($user->getId(), $userInfo->getId());

                $isUserBlocked = $em->getRepository('VocalizrAppBundle:UserBlock')
                        ->isUserBlocked($user->getId(), $userInfo->getId());
            }
        }

        $reviewRepo = $em->getRepository('VocalizrAppBundle:UserReview');
        ...
        $userAudio = $this->get('vocalizr_app.model.user_audio')->getUserAudios($userInfo);

        // Get gigs in progress
        $workInProgress = $em->getRepository('VocalizrAppBundle:Project')
                ->getProjectsInProgressByUserInvolved($userInfo->getId());

        // Get open gigs
        $openProjects = $em->getRepository('VocalizrAppBundle:Project')
        ...
        $userVideos = $this->get('vocalizr_app.model.user_video')->getUserVideos($userInfo);

        return $this->render('@VocalizrApp/User/view.html.twig', [
            'userInfo'             => $userInfo,
            ...
            'userSpotifyPlaylists' => $this->get('vocalizr_app.model.user_spotify_playlist')->getSpotifyPlaylists($userInfo, 4),
        ]);
    }
    ...
    public function viewConnectionsAction($id, Request $request)
    {
        ...
        $connectionsInviteRepository = $em->getRepository('VocalizrAppBundle:UserConnectInvite');
        ...
        $html = $this->renderView('VocalizrAppBundle:User:viewConnections.html.twig', [
     ...
     * @return bool
     */
    public function recordViewStat($userInfo)
    {
        return $this->get('vocalizr_app.service.statistics')->recordProfileViewStat($userInfo, $this->getUser());
    }
    ...
    public function editAction()
    {
        ...
        $userLanguages = $this->get('vocalizr_app.model.user_info_language')->getLanguagesByUser($user);

        $form = $this->createForm(new UserInfoType([]), $user, ['validation_groups' => ['user_edit'], 'languages' => $userLanguages]);

        $passwordForm = $this->createForm(new UserPasswordType(), [], ['validation_groups' => ['password_change']]);
        $emailForm    = $this->createForm(new UserEmailType());

        $subscriptionPlan = $em->getRepository('VocalizrAppBundle:SubscriptionPlan')->getActiveSubscription($user->getId());

        // get user preferences
        $userPref = $em->getRepository('VocalizrAppBundle:UserPref')->findOneBy([
            'user_info' => $user,
        ]);
        if (!$userPref) {
        ...
        // Get user voice tags
        $userVoiceTags = $em->getRepository('VocalizrAppBundle:UserVoiceTag')->getUserVoiceTags($user->getId());
        ...
        if ($user->isSubscribed()) {
            $userSub = $em->getRepository('VocalizrAppBundle:UserSubscription')->findOneBy([
                'user_info' => $user,
                'is_active' => 1,
            ], [
         ...
         */
        if ($user->isSubscribed() && $this->container->getParameter('pro_profile_enabled')) {
...
//                    $this->get('session')->getFlashBag()->add('success', 'Pro profile changes saved');
        ...
        if ($request->get('audio_file')) {
            ...
            if (count($userAudios) >= $subscriptionPlan['user_audio_limit']) {
                ...
                $userAudio = $this->get('vocalizr_app.model.user_audio')->saveUploadedAudio(
                    $user,
                    $title,
                    $fileName
                );
        ...
        if ($request->get('save')) {
            ...
            if ($form->isValid()) {
                ...
                $currentVocalChars = $em->getRepository('VocalizrAppBundle:UserVocalCharacteristic')
                ...
                foreach ($form->get('vocal_characteristics')->getData() as $vocalCharacteristic) {
                    ...
                    if (!in_array($vocalCharacteristic->getId(), $currentVocalChars)) {
                        $entity = new \Vocalizr\AppBundle\Entity\UserVocalCharacteristic();
                ...
                if (count($currentVocalChars) > 0) {
                    foreach ($currentVocalChars as $userVocalCharId => $vocalCharId) {
                        $em->remove($em->getReference('VocalizrAppBundle:UserVocalCharacteristic', $userVocalCharId));
                        $em->flush();
                    }
                }
                ...
                $currentVocalStyles = $em->getRepository('VocalizrAppBundle:UserVocalStyle')
                ...
                foreach ($form->get('vocal_styles')->getData() as $vocalStyle) {
                    ...
                    if (!in_array($vocalStyle->getId(), $currentVocalStyles)) {
                        $entity = new \Vocalizr\AppBundle\Entity\UserVocalStyle();
                        $entity->setUserInfo($user);
                ...
                if (count($currentVocalStyles) > 0) {
                    foreach ($currentVocalStyles as $userVocalStyleId => $vocalStyleId) {
                        $em->remove($em->getReference('VocalizrAppBundle:UserVocalStyle', $userVocalStyleId));
                        $em->flush();
                    }
                }
                /*
                ...
                $userInfo = $this->get('vocalizr_app.model.user_info_language')->setUserInfoLanguages($userInfo, $languages);
            ...
            $vocalChars = $em->getRepository('VocalizrAppBundle:VocalCharacteristic')->getByUserInfoId($user->getId());
            $choices    = new \Doctrine\Common\Collections\ArrayCollection($vocalChars);
            $form->get('vocal_characteristics')->setData($choices);

            $vocalStyles = $em->getRepository('VocalizrAppBundle:VocalStyle')->getByUserInfoId($user->getId());
        ...
        if ($request->get('save_email')) {
            ...
            if ($emailForm->isValid()) {
                ...
                $emailChangeRequestRepo = $em->getRepository('VocalizrAppBundle:EmailChangeRequest');

                $userEmail          = $em->getRepository('VocalizrAppBundle:UserInfo')->findFirstByEmail($userInfo['email']);
                ...
                if ((!$userEmail && !$emailChangeRequest) ||
                    ...
                    $em->getRepository('VocalizrAppBundle:EmailChangeRequest');
                    $emailRequest = $em->getRepository('VocalizrAppBundle:EmailChangeRequest')->findOneBy(['user_info' => $user->getId()]);
        ...
        $userAudios = array_slice($userAudios, 0, $subscriptionPlan['user_audio_limit']);

        if (!$user->getCompletedProfile()) {
            $this->checkProfileCompleteness();
        }

        return $this->render('@VocalizrApp/User/edit.html.twig', [
            'user'          => $user,
            'userVoiceTags' => $userVoiceTags,
            'form'          => $form->createView(),
            //'locationForm' => $locationForm->createView(),
            'proProfileEnabled'    => $this->container->getParameter('pro_profile_enabled'),
            ...
            'cu'                   => $cu,
            'userSub'              => $userSub,
            'userVideos'           => $this->get('vocalizr_app.model.user_video')->getUserVideos($user, 0, 10),
            'userVideosCount'      => $user->getVideosCount(),
            'userSpotifyPlaylists' => $this->get('vocalizr_app.model.user_spotify_playlist')->getSpotifyPlaylists($user, 4),
        ]);
    }
    ...
    {
        $uploadDir = $this->container->get('service.helper')->getUploadTmpDir();
        /** @var UserInfo $user */
        ...
        $this->get('vocalizr_app.model.user_info')->generateThumbnails($user);
    ...
    {
        if (!$this->container->getParameter('pro_profile_enabled')) {
    ...
    public function assignAudioTrack(Request $request)
    {
        ...
        if ($request->get('sc_id')) {
            if (!$scTrack = $em->getRepository('VocalizrAppBundle:UserScTrack')->findOneBy(['sc_id' => $request->get('sc_id')])) {
                return new JsonResponse(['error' => 'Failed to assign audio to your profile. Try uploading again.']);
            }

            $defaultTrack = true;
            $userAudio    = $em->getRepository('VocalizrAppBundle:UserAudio')->saveTrackFromSoundCloud($user->getId(), $scTrack, $defaultTrack);
        }
    ...
    public function favoritesAction(Request $request)
    {
        ...
        $form = $this->createForm(new \Vocalizr\AppBundle\Form\Type\UserSearchType());
        ...
        $q = $userInfoRepo->createQueryBuilder('ui')
            ...
            ->where('ui.id IN (SELECT f.id FROM VocalizrAppBundle:UserInfo ui2 INNER JOIN ui2.favorites f WHERE ui2.id = :userInfoId)')
        ...
        if ($userIds) {
            // See if logged in member has any of those connections
            $qb = $em->getRepository('VocalizrAppBundle:UserConnectInvite')
            ...
            $qb = $em->getRepository('VocalizrAppBundle:UserAudio')
    ...
    public function blockAction(Request $request)
    {
        ...
        $user          = $this->getUser();
        $userInfoRepo  = $em->getRepository('VocalizrAppBundle:UserInfo');
        $userBlockRepo = $em->getRepository('VocalizrAppBundle:UserBlock');
        ...
        $qb->delete('VocalizrAppBundle:ProjectBid', 'pb');
        $qb->where('pb.user_info = :user AND pb.flag IS NULL');
        $qb->setParameter('user', $user);
        $qb->getQuery()->execute();

        // add them
        $obj = new \Vocalizr\AppBundle\Entity\UserBlock();
    ...
    public function userTagVoteAction(Request $request)
    {
        ...
        if ($type == 'voiceTag') {
            ...
            $userVoiceTag = $em->getRepository('VocalizrAppBundle:UserVoiceTag')
            ...
            if ($userVoiceTag && $userVoiceTag->getUserInfo()->getId() != $user->getId()) {
                ...
                if (count($userVoiceTag->getUserVoiceTagVotes()) == 0) {
                    ...
                    // Add vote
                    $vote = new \Vocalizr\AppBundle\Entity\UserVoiceTagVote();
        ...
        if ($type == 'vocalStyle') {
            $userVocalStyle = $em->getRepository('VocalizrAppBundle:UserVocalStyle')
                    ->getByIdJoinVotedUser($id, $user->getId());
            // Does tag exist and make sure it's not the logged in user trying
            // to vote for themself
        ...
        if ($type == 'vocalCharacteristic') {
            $userVocalCharacteristic = $em->getRepository('VocalizrAppBundle:UserVocalCharacteristic')
                    ->getByIdJoinVotedUser($id, $user->getId());
            // Does tag exist and make sure it's not the logged in user trying
            // to vote for themself
    ...
    public function audioAction()
    {
        ...
        $userAudio = $em->getRepository('VocalizrAppBundle:UserAudio')->findOneBy(['slug' => $request->get('slug')]);
    ...
    public function upgradeAction(Request $request)
    {
        ...
        $proPlan = $em->getRepository('VocalizrAppBundle:SubscriptionPlan')->findOneBy([
            'static_key' => 'PRO',
        ]);
        ...
        if ($request->isMethod('POST') && $currentPlan['static_key'] != 'PRO') {
            $newPlan      = $em->getRepository('VocalizrAppBundle:SubscriptionPlan')->findOneBy(['static_key' => 'PRO']);
            ...
            $userSubscription = new \Vocalizr\AppBundle\Entity\UserSubscription();
        ...
        $userSub = $em->getRepository('VocalizrAppBundle:UserSubscription')->findOneBy(['user_info' => $this->getUser()]);
        ...
        return $this->render('@VocalizrApp/User/upgrade.html.twig', $data);
    }
     ...
     * @return Response
     */
    public function connectRequestAction(Request $request)
    {
        $em                    = $this->getDoctrine()->getManager();
        /** @var UserInfo $user */
        $user                  = $this->getUser();
        $userConnectInviteRepo = $em->getRepository('VocalizrAppBundle:UserConnectInvite');

        // Get user who we are wanting to connect with
        $toUser = $em->getRepository('VocalizrAppBundle:UserInfo')->findOneBy([
        ...
        if ($violations) {
            ...
            if ($violations->hasViolation(UserConnectModel::CONSTRAINT_LIMIT_REACHED)) {
                ...
                return $this->render('@VocalizrApp/User/include/extend_connections.html.twig', [
                    'options' => $priceValues,
                ], new Response('', 400));
            }
        ...
        return $this->render('@VocalizrApp/User/connectRequest.html.twig', [
            'toUser'           => $toUser,
            'subscriptionPlan' => $user->getSubscriptionPlan(),
            'defaultMessage'   => $this->container->getParameter('connect_default_msg'),
    ...
    public function connectAcceptAction(Request $request)
    {
        ...
        $em->getRepository('VocalizrAppBundle:Notification')
     ...
     * @Template()
     */
    public function dontCancelMembershipModalAction()
    {
        $em   = $this->getDoctrine()->getManager();
        $user = $this->getUser();
     ...
     * @Template()
     */
    public function cancelModalAction()
    {
        $em   = $this->getDoctrine()->getManager();
        $user = $this->getUser();
    ...
    public function getUserVideosAction(Request $request)
    {
        ...
        if (count($userVideos) > 0) {
            if ($request->get('edit')) {
                $html = $this->renderView('VocalizrAppBundle:User:edit_videos.html.twig', [
                    'userVideos' => $userVideos,
                ]);
            } else {
                $html = $this->renderView('VocalizrAppBundle:User:view_videos.html.twig', [
                    'userVideos' => $userVideos,
                ]);
            }
        } else {
    ...
    public function addUserSpotifyPlaylistAction(Request $request)
    {
        ...
        $html = $this->renderView('VocalizrAppBundle:User:user_spotify_playlists.html.twig', [
    ...
    public function getMoreUserReviewAction(Request $request)
    {
        ...
        $reviewsQuery = $this->getDoctrine()->getRepository('VocalizrAppBundle:UserReview')->getUserReviewsByTypeQb($user, $type);
        ...
        $html = $this->renderView('VocalizrAppBundle:User:view_reviews.html.twig', [
            'reviews' => $pagination,
        ]);
    ...
    public function _projectsAwardedNeedResponseAction()
    {
        $user            = $this->getUser();
        $em              = $this->getDoctrine()->getManager();
        $projectsAwarded = $em->getRepository('VocalizrAppBundle:ProjectBid')
