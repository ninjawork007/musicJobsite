...
class VocalistSearchType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
         ...
         *              Get genres
         */
        $results = $em->getRepository('VocalizrAppBundle:Genre')
                ->getAll();
        $genres = [];
        if ($results) {
        ...
        // Get vocal chars
        $results = $em->getRepository('VocalizrAppBundle:VocalCharacteristic')
                ->getAll();
        $vocalChars = [];
        if ($results) {
            foreach ($results as $result) {
        ...
        // Get vocal styles
        $results = $em->getRepository('VocalizrAppBundle:VocalStyle')
                ->getAll();
        $vocalStyles = [];
        if ($results) {
            foreach ($results as $result) {
