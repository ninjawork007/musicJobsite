...
class PayPalService
{
     ...
     */
    public function __construct(EntityManagerInterface $doctrine, ContainerInterface $container, MandrillService $mandrill, Dispatcher $dispatcher, Environment $templating)
    {
        $this->em         = $doctrine;
    ...
    public function processIpn($data = null)
    {
        ...
        $subscriptionPlanRepo     = $em->getRepository('App:SubscriptionPlan');
        $userInfoRepo             = $em->getRepository('App:UserInfo');
        $userSubscriptionPlanRepo = $em->getRepository('App:UserSubscription');
    ...
    public function handleWebAccept($data)
    {
        ...
        if (substr($data['custom'], 0, 7) == 'DEPOSIT') {
            ...
            $user = $this->em->getRepository('App:UserInfo')->findOneBy(['unique_str' => $userStr]);
            if (!$user) {
        ...
        if (substr($data['custom'], 0, 6) == 'ENGINE') {
            ...
            $engineOrder = $this->em->getRepository('App:EngineOrder')->findOneBy(['uid' => $uid]);
...
//        $uniqueKey = $data['item_number'];
    ...
    public function handleSubscrCancel($data, $atPeriodEnd = true)
    {
        ...
        $subscriptionPlanRepo = $em->getRepository('App:SubscriptionPlan');
        $userInfoRepo         = $em->getRepository('App:UserInfo');
        $userSubscriptionRepo = $em->getRepository('App:UserSubscription');
    ...
    {
        $userInfoRepo = $this->em->getRepository('App:UserInfo');
...
//        $mailer->sendMessage($userInfo->getEmail(), '');
    ...
    private function resolveWalletTransaction($itemData)
    {
        ...
        $transaction = $this->em->getRepository('App:UserWalletTransaction')
    ...
    private function resolveWithdraw($itemData)
    {
        ...
        $withdraw = $this->em->getRepository('App:UserWithdraw')
            ->find($withdrawId);
