...
class HelperService
{
     ...
     */
    public function __construct(EntityManagerInterface $manager, ContainerInterface $container, Environment $templating)
    {
        $this->em         = $manager;
    ...
    {
        return $this->container->getParameter('kernel.project_dir') . '/tmp';
    }

    public function getWaveformDir()
    {
        return $this->container->getParameter('kernel.project_dir') . '/public/plugins/waveform';
    }
    ...
    {
        if (strtoupper(substr(PHP_OS, 0, 3)) == 'WIN') {
            pclose(popen('start /B php ' . $this->container->getParameter('kernel.project_dir') . '/console ' . $cmd, 'r'));
        } else {
            $phpPath = exec('which php');
            $cmd     = $phpPath . ' ' . $this->container->getParameter('kernel.project_dir') . '/console ' . $cmd . ' > /dev/null &';
            exec($cmd);
        }
    }
