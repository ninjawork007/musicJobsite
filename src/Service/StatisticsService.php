...
class StatisticsService
{
    ...
    public function recordProfileViewStat(UserInfo $profileOwner, UserInfo $currentUser = null)
    {
        ...
        // Record view stat
        $profileViewRepo = $this->dm->getRepository('App:ProfileView');
        ...
        if ($uniqueView) {
            ...
            if (!$currentUser) {
                ...
                $profileViewUser = new \App\Document\ProfileViewUser();
        ...
        if ($currentUser) {
            $pVURepo = $this->dm->getRepository('App:ProfileViewUser');
            $params  = [
                'user_id'      => $profileOwner->getId(),
