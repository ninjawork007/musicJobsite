 ...
 */
class StripeSessionController extends AbstractController
{
    /**
     ...
     */
    public function getStripeCheckoutSessionForProject(Request $request, ProjectPriceCalculator $priceCalculator, StripeConfigurationProvider $configuration, StripeManager $stripeManager)
    {
        ...
        /** @var Project $project */
        $project = $em->getRepository('App:Project')->findOneBy(['uuid' => $uuid]);
...
//        $stripeManager = $this->get('vocalizr_app.stripe_manager');
     ...
     *
     */
    public function createUserUpgradeStripeSession(Request $request, StripeConfigurationProvider $config, StripeManager $stripe)
    {
        $key    = $request->get('key');
...
//        $stripe = $this->get('vocalizr_app.stripe_manager');
     ...
     */
    public function createExtendContestStripeSession(Request $request, StripeConfigurationProvider $configurationProvider, StripeManager $stripeManager)
    {
        $em = $this->getDoctrine()->getManager();
        $uuid = $request->get('uuid');
        ...
        try {
            $daysPriceIdsMap = $configurationProvider->getProductPrices('extend_contest');
            ...
            $stripeResponseData = $stripeManager->getStripeSession($user, [[
                'price'    => $priceId,
                'quantity' => 1,
            ]], $returnUrl);
     ...
     *
     * @param StripeManager $stripeManager
     * @param StripeConfigurationProvider $stripeConfigurationProvider
     * @param MembershipSourceHelper $membershipSourceHelper
     * @return JsonResponse
     */
    public function getStripeSubscriptionSession(Request $request, StripeManager $stripeManager, StripeConfigurationProvider $stripeConfigurationProvider, MembershipSourceHelper $membershipSourceHelper)
    {
...
//        $stripeManager = $this->get('vocalizr_app.stripe_manager');
        ...
        $priceId = $stripeConfigurationProvider->getSubscriptionPriceId($request->get('plan'));
        ...
        $source = $membershipSourceHelper->getSource(false);
     ...
     * @return Response
     */
    public function createBidUpgradeStripeSession(Request $request, StripeConfigurationProvider $config, StripeManager $stripe)
    {
        $em      = $this->getDoctrine()->getManager();
        $user    = $this->getUser();
        $option  = $request->get('option');
//        $config  = $this->get('vocalizr_app.stripe_configuration_provider');
        ...
        /** @var ProjectBid $bid */
        $bid     = $em->getRepository('App:ProjectBid')->getProjectBidByUuid($request->get('uuid'));
        $bid     = reset($bid);
