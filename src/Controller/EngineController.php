...
use App\Entity\EngineOrder;
use App\Form\Type\EngineOrderType;
...
class EngineController extends AbstractController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $products = $em->getRepository('App:EngineProduct')->findBy(['pro_only' => false], ['sort_order' => 'ASC']);

        $proResults = false;
        if ($user && $user->isSubscribed()) {
            // Get only pro products
            $proResults = $em->getRepository('App:EngineProduct')->findBy(['pro_only' => true], ['sort_order' => 'ASC']);
        }
    ...
    public function editAction(Request $request)
    {
        ...
        $engineOrder = false;
        if ($uid) {
            $engineOrder = $em->getRepository('App:EngineOrder')->findOneBy(['uid' => $uid]);
        ...
        if ($code) {
            ...
            if ($user->isSubscribed()) {
                ...
                $proProduct = $em->getRepository('App:EngineProduct')->findOneBy(['code' => $code . '-PRO']);
            }

            $product = $em->getRepository('App:EngineProduct')->findOneBy(['code' => $code]);
        ...
        if (count($files) > 0 || count($dbfiles) > 0) {
            ...
            if (count($files) > 0) {
                foreach ($files as $i => $file) {
                    ...
                    if (!$engineOrderAsset = $em->getRepository('App:EngineOrderAsset')
            ...
            if ($dbFileNames = $request->get('dropbox_file_name')) {
                ...
                foreach ($dbFileNames as $k => $name) {
                    ...
                    $engineOrderAsset = $em->getRepository('App:EngineOrderAsset')
    ...
    public function deleteAssetAction(Request $request, $uid, $slug)
    {
        ...
        $engineOrder = $em->getRepository('App:EngineOrder')->findOneByUid($uid);
        if (!$engineOrder) {
        ...
        $asset = $em->getRepository('App:EngineOrderAsset')->getBySlugAndEngineOrder($slug, $engineOrder);
