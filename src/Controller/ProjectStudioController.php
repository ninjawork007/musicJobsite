...
// Forms
use App\Form\Type\ProjectDisputeType;
use App\Form\Type\ProjectLyricType;
use App\Form\Type\UserReviewType;
 ...
 */
class ProjectStudioController extends AbstractController
{
    /**
    ...
    public function indexAction(Request $request)
    {
        ...
        if ($project->getIsComplete()) {
            $userReview = $em->getRepository('App:UserReview')->findOneBy([
                'project'   => $project->getId(),
         ...
         */
        $projectDisputes = $em->getRepository('App:ProjectDispute')
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($request->get('comment') || $request->get('audio_file')) {
                ...
                if (!isset($audioError)) {
                    $projectComment = new \App\Entity\ProjectComment();
            ...
            if ($request->get('save_lyrics')) {
                ...
                if ($lyricForm->isValid()) {
                    ...
                    $projectLyrics = new \App\Entity\ProjectLyrics();
            ...
            if ($request->get('review') && $canReview) {
                ...
                if ($userReviewForm->isValid()) {
                    ...
                    $otherUserReview = $em->getRepository('App:UserReview')->findOneBy([
                        'project'   => $project->getId(),
        ...
        // Get project feed
        $projectFeed = $em->getRepository('App:ProjectFeed')
                ->getFeed($project->getId());

        $feedAudios = $em->getRepository('App:ProjectAudio')->getFeedAudio(
            $projectFeed,
            $project->getId()
        );

        // Get lyrics history
        $projectLyrics = $em->getRepository('App:ProjectLyrics')
                ->getLyrics($project->getId());

        // Get project assets
        $projectAssets = $em->getRepository('App:ProjectAsset')
    ...
    public function _studioActionSubmitDispute()
    {
        ...
        $disputeActive = $this->em->getRepository('App:ProjectDispute')->findOneBy(['project' => $this->project, 'accepted' => null]);
    ...
    public function _studioActionDisputeResponse()
    {
        ...
        $dispute = $this->em->getRepository('App:ProjectDispute')
                ->findOneBy([
        ...
        if (count($files) > 0) {
            ...
            if (count($files) > 0) {
                foreach ($files as $i => $file) {
                    ...
                    if (
                        !$projectAsset = $this->em->getRepository('App:ProjectAsset')
    ...
    private function _studioActionReleaseEscrow()
    {
        ...
        $bidderSubscriptionPlan = $this->em->getRepository('App:SubscriptionPlan')
...
//        ], true), $this->project->getInvoicePdfPath());
    ...
    public function disputeDeleteAction(Request $request)
    {
        ...
        // Get project dispute
        $dispute = $em->getRepository('App:ProjectDispute')
                ->findOneBy([
                    'id'             => $id,
                    'from_user_info' => $user,
                ]);
    ...
    public function feedAction(Request $request)
    {
        ...
        // Make all unread feed items as read if action is done by other party
        //$em->getRepository('App:ProjectFeed')
    ...
    public function downloadMasterAction(Request $request, $uuid, $slug)
    {
        ...
        $projectAudio = $em->getRepository('App:ProjectAudio')
                    ->findOneBy([
                        'slug' => $slug,
                    ]);
    ...
    public function downloadAssetsAction(Request $request, $uuid)
    {
        ...
        /** @var Project $project */
        $project = $em->getRepository('App:Project')->getProjectByUuid($uuid);
