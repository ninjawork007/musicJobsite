 ...
 * @package App\Command
 */
class FixDoubleBillingCommand extends Command
{
    /**
     * @var EntityManager
     */
    ...
    {
        $qb = $this->em->getRepository('App:UserWithdraw')->createQueryBuilder('w');
        $qb
            ->where('w.id in (:ids)')
    ...
    {
        $qb = $this->em->getRepository('App:UserWalletTransaction')->createQueryBuilder('t');
        $qb
